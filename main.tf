...
module "secret_manager" {
  ...
  name = each.key
  env_vars   = local.env_vars[each.value.app_name]
...
module "ecs_services" {
  ...
  ecs_secret_arn = local.secret_arns["${each.value.app_name}-secrets"]
